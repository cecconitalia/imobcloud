  ...
  <div class="page-container">
    ...
    <header class="dash-header">
      ...
      <div class="header-actions">
        ...
        <button @click="refreshData" class="btn-icon-refresh" title="Atualizar Dashboard" :disabled="isLoading">
            <i class="fas fa-sync-alt" :class="{ 'fa-spin': isLoading }"></i>
        </button>
      </div>
    </header>

    <div v-if="isLoading && stats.total_oportunidades === -1" class="loading-skeleton">
    ...
    <div v-else class="dashboard-content">
        ...
        <section class="kpi-grid">
            <div class="kpi-card" @click="$router.push({ name: 'funil-vendas' })" style="cursor: pointer;">
                ...
                <div class="kpi-data">
                    <span class="kpi-value">{{ stats.oportunidades_ativas }}</span>
            ...
            <div class="kpi-card" @click="$router.push({ name: 'tarefas-board' })" style="cursor: pointer;">
                ...
                <div class="kpi-data">
                    <span class="kpi-value">{{ stats.tarefas_pendentes }}</span>
            ...
            <div v-if="canViewFinance" class="kpi-card" @click="$router.push({ name: 'financeiro-dashboard' })" style="cursor: pointer;">
                ...
                <div class="kpi-data">
                    <span class="kpi-value">{{ formatCurrency(stats.receita_mes) }}</span>
            ...
            <div v-if="canViewImoveis" class="kpi-card" @click="$router.push({ name: 'imoveis' })" style="cursor: pointer;">
                ...
                <div class="kpi-data">
                    <span class="kpi-value">{{ stats.imoveis_ativos }}</span>
        ...
        <div class="main-grid">
            ...
            <div class="grid-column">
                        ...
                        <ul v-else class="task-list">
                            <li v-for="task in stats.tarefas_hoje" :key="task.id" class="task-item">
                                ...
                                <div class="task-info">
                                    ...
                                    <span class="task-meta">{{ formatTime(task.data_vencimento) }} &bull; {{ task.cliente_nome || 'Geral' }}</span>
                                </div>
                            </li>
                        </ul>
                    </div>
                ...
                <div v-if="canViewFinance" class="widget-card mt-4">
                    <div class="widget-header">
                        <h3><i class="fas fa-chart-line text-green"></i> Previsão Financeira</h3>
                        <router-link :to="{ name: 'financeiro-dashboard' }" class="link-small">Detalhes</router-link>
                    </div>
                    <div class="widget-body finance-summary">
                        <div class="fin-row">
                            <span>Entradas Previstas</span>
                            <span class="fin-val text-green">{{ formatCurrency(stats.financeiro_previsto_entrada) }}</span>
                        </div>
                        <div class="fin-row">
                            <span>Saídas Previstas</span>
                            <span class="fin-val text-red">{{ formatCurrency(stats.financeiro_previsto_saida) }}</span>
                        </div>
                        <div class="fin-divider"></div>
                        <div class="fin-row total">
                            <span>Saldo Projetado</span>
                            <span class="fin-val">{{ formatCurrency(stats.financeiro_previsto_entrada - stats.financeiro_previsto_saida) }}</span>
                        </div>
                    </div>
                </div>
            ...
            <div class="grid-column">
                ...
                <div class="widget-card">
                    ...
                    <div class="widget-body">
                        <div v-if="!stats.proximas_visitas || stats.proximas_visitas.length === 0" class="empty-widget">
                        ...
                        <div v-else class="visit-list">
                            <div v-for="visita in stats.proximas_visitas" :key="visita.id" class="visit-item">
                                ...
                                <div class="visit-details">
                                    ...
                                    <span class="text-xs text-gray-500">{{ formatTime(visita.data) }}</span>
                                </div>
                            </div>
                        </div>
                    </div>
...
interface DashboardStats {
    ...
    financeiro_previsto_entrada: number;
    financeiro_previsto_saida: number;
...
const refreshData = async () => {
    ...
    // URLs Corrigidas: v1/ adicionado manualmente
    const promises = [
        api.get('v1/oportunidades/stats/'),
        api.get('v1/tarefas/stats/'),
        api.get('v1/imoveis/stats/'),
        api.get('v1/financeiro/stats/')
    ];

    try {
        const results = await Promise.allSettled(promises);

        // 1. Funil / Oportunidades
        if (results[0].status === 'fulfilled') {
            const data = results[0].value.data;
            stats.value.oportunidades_ativas = data.total_ativas || 0;
            stats.value.total_oportunidades = data.total_ativas || 0;
            
            if (data.fases_detalhadas) {
                stats.value.funil_resumo = data.fases_detalhadas;
            } else if (data.total_ativas > 0) {
                stats.value.funil_resumo = [{ titulo: 'Ativos', total: data.total_ativas }];
            } else {
                stats.value.funil_resumo = [];
            }
        }
        ...
        if (results[1].status === 'fulfilled') {
            ...
            stats.value.tarefas_pendentes = data.pendentes || 0;
            stats.value.tarefas_hoje = data.hoje || [];
        }

        // 3. Imóveis e Visitas
        if (results[2].status === 'fulfilled') {
            const data = results[2].value.data;
            stats.value.imoveis_ativos = data.ativos || 0;
            
            // CORREÇÃO: Forçar atualização mesmo se vier vazio ou null
            // Debug: console.log('Visitas recebidas:', data.visitas_agendadas);
            stats.value.proximas_visitas = data.visitas_agendadas || [];
        } else {
            console.error("Erro ao carregar Imóveis Stats:", results[2].reason);
        }

        // 4. Financeiro
        if (results[3].status === 'fulfilled') {
            const data = results[3].value.data;
            stats.value.receita_mes = data.receita_mes || 0;
            stats.value.financeiro_previsto_entrada = data.previsto_entrada || 0;
            stats.value.financeiro_previsto_saida = data.previsto_saida || 0;
        }

    } catch (error) {
        console.error("Erro crítico no dashboard:", error);
    } finally {
        if (stats.value.total_oportunidades === -1) stats.value.total_oportunidades = 0;
...
.btn-icon-refresh:disabled { opacity: 0.7; cursor: not-allowed; }
.kpi-grid {
