<template>
  <div class="page-container">
      ...
      <div class="header-main">
        <div class="title-area">
           <nav class="breadcrumb">
              ...
              <span class="active">Contas a Pagar</span>
           </nav>
        ...
        <div class="actions-area">
            ...
            <router-link :to="{ name: 'transacao-nova', query: { tipo: 'DESPESA' } }" class="btn-primary-thin">
    ...
    <div class="kpi-grid" v-if="stats && stats.a_pagar">
      <div class="kpi-card orange">
        <div class="kpi-content">
          <span class="kpi-value">{{ formatarValor(stats.a_pagar.pendente) }}</span>
          <span class="kpi-label">A Pagar (Pendente)</span>
        </div>
      ...
      <div class="kpi-card green">
        <div class="kpi-content">
          ...
          <span class="kpi-label">Pago (Mês Atual)</span>
        </div>
      ...
      <div class="kpi-card purple">
        <div class="kpi-content">
          ...
          <span class="kpi-label">Contas Listadas</span>
        </div>
    ...
    <main class="report-main-wrapper">
      ...
      <div v-else-if="error" class="empty-state">
        <i class="fas fa-exclamation-circle empty-icon text-red"></i>
        <p>{{ error }}</p>
      </div>

      <div v-else-if="transacoes.length === 0" class="empty-state">
        <i class="fas fa-folder-open empty-icon"></i>
        <p>Nenhuma conta a pagar encontrada para os filtros selecionados.</p>
      </div>
          ...
          <thead>
            <tr>
              ...
              <th width="25%" @click="ordenar('cliente__nome')" class="sortable">
                  Fornecedor/Destino <i class="fas" :class="getIconeOrdenacao('cliente__nome')"></i>
              </th>
          ...
          <tbody>
            <tr v-for="transacao in transacoes" :key="transacao.id">
              ...
              <td class="font-medium text-slate-700">
                  {{ transacao.fornecedor_nome || transacao.cliente_nome || 'Não informado' }}
              </td>
    ...
    <div v-if="showModal" class="modal-overlay" @click.self="fecharModal">
      <div class="modal-content">
        ...
        <div class="modal-body" v-if="selectedTransacao">
          <div class="summary-box">
             <div class="summary-item">
                ...
                <span class="value">{{ selectedTransacao.descricao }}</span>
             </div>
             <div class="summary-item">
                <span class="label">Valor</span>
                <span class="value highlight text-danger">{{ formatarValor(selectedTransacao.valor) }}</span>
             </div>
          </div>
        ...
        <div class="modal-footer">
          ...
          <button class="btn-primary-modal" @click="confirmarPagamento" :disabled="isProcessing">
...
const reverterPagamento = async (transacao: TransacaoPagar) => {
    if (!confirm(`Deseja reverter o pagamento de "${transacao.descricao}" para PENDENTE?`)) return;
...
async function fetchData(isFilterChange: boolean = false) {
  ...
  try {
    ...
    const transacoesResponse = await apiClient.get<PaginatedResponse<TransacaoPagar>>(
...
.page-container {
  min-height: 100vh; /* Permite que a página cresça e use a rolagem do navegador */
  display: flex;
...
/* TABELA E LAYOUT */
.report-main-wrapper {
  /* Remove flex: 1 e overflow: hidden para permitir o Page Scroll */
  width: 100%;
...
.report-scroll-viewport { 
    width: 100%; overflow-x: auto; /* Apenas rolagem horizontal se necessário */
...
.report-table th {
  ...
  /* Sticky header relativo à janela */
...
.summary-item .highlight { color: #e11d48; font-size: 1.1rem; }
