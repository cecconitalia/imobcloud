<template>
  <div class="page-container">
    <div class="header-section">
      ...
      <p class="page-subtitle">Gestão de despesas pendentes e pagas.</p>
    </div>
      ...
      <div class="toolbar">
        <div class="filters-wrapper">
          <div class="filter-group search-filter">
            ...
            <input type="text" v-model="filters.search" placeholder="Descrição, fornecedor, contrato..." class="search-input" />
          </div>
...
interface Stats { total_a_pagar: number; total_vencido: number; quantidade_vencidas: number; }
...
const fetchCategorias = async () => {
  try {
    const response = await api.get('/v1/financeiro/categorias/?tipo=DESPESA'); // Apenas categorias de despesa
...
const fetchData = async () => {
  ...
  try {
    ...
    const response = await api.get('/v1/financeiro/transacoes/a-pagar/', { params });
...
<style scoped>
/* Estilos são os mesmos do componente anterior, focados em layout e apresentação */
...
.action-button { padding: 6px 12px; border-radius: 5px; border: none; cursor: pointer; font-weight: 500; color: white; background-color: #dc3545; }
