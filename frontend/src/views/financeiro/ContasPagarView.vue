...
function formatarValor(valor: number | null | undefined): string { // Aceita undefined
...
async function fetchStats() {
  ...
  try {
    ...
    const response = await apiClient.get<StatsFinanceiro>('/v1/financeiro/transacoes/stats/'); // Tipagem aplicada
    stats.value = response.data;
  } catch (err) {
    console.error("Erro ao buscar estatísticas financeiras:", err);
    // Mensagem de erro que você reportou (pode indicar problema na API ou na lógica de exibição)
...
.error-message {
  color: red; /* Mantido para destacar o erro */
  /* Adiciona algum destaque visual se preferir */
